...
import os
import idr_torch  # Add this import
...
def print_distributed_info():
    print(f"RANK: {idr_torch.rank}, "  # Modified
          f"WORLD_SIZE: {idr_torch.world_size}, "  # Modified
          f"LOCAL_RANK: {os.environ.get('LOCAL_RANK', 'Not set')}, "
          f"NODE_RANK: {os.environ.get('NODE_RANK', 'Not set')}")
...
def get_dataloader_from_data_stage(
    ...
    if consumed_train_samples is None:
        # If consumed_train_samples is None, calculate it based on the current iteration step
        consumed_train_samples = trainer.iteration_step * trainer.global_batch_size
        log_rank(
            f"Recalculated consumed_train_samples: {consumed_train_samples}",
            logger=logger,
...
def get_dataloader(trainer: DistributedTrainer) -> Dict[str, DataLoader]:
    ...
    for stage_idx, stage in enumerate(trainer.config.data_stages):
        ...
        if consumed_train_samples is None:
            # If consumed_train_samples is None, use the current iteration step to estimate
            consumed_train_samples = max(0, trainer.iteration_step - stage.start_training_step) * trainer.global_batch_size
            log_rank(
                f"Estimated consumed_train_samples for stage {stage.name}: {consumed_train_samples}",
                logger=logger,
                level=logging.WARNING,
                rank=0,
            )
...
if __name__ == "__main__":
    ...
    print("About to initialize DistributedTrainer...")
    print("DistributedTrainer initialization starting...")
    ...
    print("DistributedTrainer initialized.")
    print("DistributedTrainer initialization complete.")
